...
| UI | shadcn/ui `radix-nova` | CSS vars: oklch in `app.css`, hsl in `tailwind.config.js`; `@tailwindcss/forms` in devDeps but **not** in plugins |
...
`auth.user` — `{ id, name, email, roles: string[], notifications, unread_count }` or `null`. Roles is a plain array from `$user->getRoleNames()`.
...
- **Notificiations** — `SystemNotification($title, $body, $url, $type)` via `$user->notify(...)` (database channel only); polled on frontend via `auth.user.notifications`
- **Messaging** — poll-based (no broadcasting); `GET /messages/{conversation}/poll`
...
- **JSON casts** — `AlumniProfile.skills`, `JobPosting.requirements`, `TracerStudyForm.questions`, `ForumTopic.attachment` are JSON arrays; `JobApplication.interview_details` is JSON; `JobPosting.is_active` is boolean
- **User `$fillable`** — PHP 8 `#[Fillable(['name', 'email', 'password'])]`; `role` column is NOT fillable (DB column, set manually in seeder, redundant with Spatie roles)
- **`JobPosting`** — Has both `$guarded = []` and `$fillable` (all columns mass-assignable)
